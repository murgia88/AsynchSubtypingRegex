...
object Main:
        @main def run(@arg(doc="Candidate subtype file",positional = true)
                      file1: String,
                      @arg(doc="Candidate supertype file",positional = true)
                      file2: String,
        ...
        var type2 = scala.io.Source.fromFile(new File(file2)).mkString.mkString.replaceAll("\\s","");
        ...
        p.parse(p.sessionType, type1) match
          	case p.Success(result, next) => {
          		...
          		if result.isWellFormed() then{
          			...
          			val T1 = result.toAutomaton()
          			p.parse(p.sessionType, type2) match
          				case p.Success(result, next) => {
          					println(file2 + " parsed")
          					if result.isWellFormed() then{
          						println(file2 + " is well formed")
          						val T2 = result.toAutomaton()
          						if dual.value then println(SubtypeAndOutputGraph(T2.dual(),T1.dual(),dir))
          						else println(SubtypeAndOutputGraph(T1,T2,dir))
          					}
          					else println(file2 + " is not well formed. Terminating...")
          				}
          				case _ => {
          				  println(file2 + " not parsed. Terminating...")	  
          				}
          		}else println(file1 + " is not well formed. Terminating...")
          	}
          	case _ => {
          	                println(file1 + " not parsed. Terminating...")	  
          	}
        }
...
//def TestMultiparty = {
//TestList(List (("TCTX*.txt","TCTX.txt")),"multipartyLog.txt",true,true,true,false)
//}

//def TestMy = {
//TestList(List (("ex1.txt","ex2.txt")),"prova.txt",false,true,true,false)
//}
